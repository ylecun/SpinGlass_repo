...
\[Y \!=\! q\!\!\!\!\sum_{i_1,i_2,\dots,i_H=1}^{\Lambda}\!\!\!\left(r_{i_1,i_2,\dots,i_H}X_{i_1,i_2,\dots,i_H}A_{i_1,i_2,\dots,i_H}\right)\prod_{k = 1}^{H}w_{i_k},
\]
...
\begin{equation}
Y_{\Lambda} := \mathbb{E}_A[Y] = q\!\!\!\!\!\sum_{i_1,\dots,i_H=1}^{\Lambda}\!\!\!\!\!\!\!\left(r_{i_1,\dots,i_H}X_{i_1,\dots,i_H}\right)\rho\prod_{k = 1}^{H}\!\!w_{i_k}.
\label{eq:befapprox}
\end{equation}
For our theoretical analysis we also assume that $X$'s are independent identically distributed such that $X_{i_1,i_2,\dots,i_H}^{(c)}\sim\mathcal{N}(0,1)$. This assumption is simplistic, yet pervasively used in machine learning theory (plethora of decoupling strategies are discussed in~\cite{opac-b1095246}). Finally, let $Y_{\Lambda} = \mathbb{E}_A[Y]$, where in particular $\Lambda$ in this notation refers to the size of set $\mathcal{A}$ (this notation will be helpful later in this section).
...
\begin{equation}
(1-\delta)C\sqrt{\frac{\Psi}{\Lambda^H}} \leq r_{i_1,i_2,\dots,i_H} \leq (1+\delta)C\sqrt{\frac{\Psi}{\Lambda^H}},
\label{eq:uniform}
\end{equation}
where the factor $\sqrt{\frac{\Psi}{\Lambda^H}}$ comes from the fact that for the network where every weight is uniformly distributed on the graph of connections (thus every node has equal probability of being adjacent to an edge with any of the unique weights) it holds that $r_{i_1,i_2,\dots,i_H} = C\sqrt{\frac{\Psi}{\Lambda^H}}$. Note that this condition can be sattified with small $\delta$ (close to $0$) if $s$ is sufficiently small.

Consider therefore an expression as follows
\begin{equation}
\hat{Y}_s = Cq\sum_{i_1,i_2,\dots,i_H=1}^{\Lambda}\sqrt{\frac{\Psi}{\Lambda^H}}X_{i_1,i_2,\dots,i_H}\rho\prod_{k = 1}^{H}w_{i_k},
\label{eq:approx}
\end{equation}
...
&=& \!\!\!\!\!\rho^{'} + \rho^{'}\hat{Y}_s\\
&=& \!\!\!\!\!\rho^{'} + Cq\sqrt{\!\frac{\Psi}{\Lambda^H}\!}\!\!\!\sum_{i_1,i_2,\dots,i_H=1}^{\Lambda}\!\!\!\!\!Z_{i_1,i_2,\dots,i_H}\rho\rho^{'}\prod_{k = 1}^{H}w_{i_k}
\end{eqnarray*}
Let $\tilde{w}_i = \sqrt[H]{\rho\rho^{'}}w_i$ for all $i = \{1,2,\dots,k\}$.  Thus
\begin{equation}
\mathcal{L}_{\Lambda,H}({\bm w}) = \rho + Cq\sqrt{\frac{\Psi}{\Lambda^H}}\!\!\sum_{i_1,\dots,i_H=1}^{\Lambda}\!\!Z_{i_1,\dots,i_H}\prod_{k = 1}^{H}\tilde{w}_{i_k}.
\label{eq:befloss}
\end{equation}
...
\[\mathbb{E}[\hat{Y}_sY_{\Lambda}] = C^2q^2\rho^2\sum_{i_1,i_2,\dots,i_H=1}^{\Lambda}\sqrt{\frac{\Psi}{\Lambda^H}r_{i_1,i_2,\dots,i_H}^2}\prod_{k = 1}^{H}w_{i_k}^2
\]
and
\[std(\hat{Y}_s) = Cq\rho\sqrt{\sum_{i_1,i_2,\dots,i_H=1}^{\Lambda}\frac{\Psi}{\Lambda^H}\prod_{k = 1}^{H}w_{i_k}^2}
\]
\[std(Y_{\Lambda}) = Cq\rho\sqrt{\sum_{i_1,i_2,\dots,i_H=1}^{\Lambda}r_{i_1,i_2,\dots,i_H}^2\prod_{k = 1}^{H}w_{i_k}^2}.
\]
Therefore
\begin{eqnarray*}
&&\!\!\!\!\!\!\!\!\!corr(\hat{Y}_s,Y_{\Lambda})\\ 
&\!\!\!\!\!\!\!\!\!=&\!\!\!\!\!\!\! \frac{\displaystyle\sum_{i_1,\dots,i_H=1}^{\Lambda}\sqrt{\frac{\Psi}{\Lambda^H}r_{i_1,\dots,i_H}^2}\prod_{k = 1}^{H}w_{i_k}^2}{\sqrt{\!\!\!\left(\displaystyle\sum_{i_1,\dots,i_H=1}^{\Lambda}\frac{\Psi}{\Lambda^H}\prod_{k = 1}^{H}w_{i_k}^2\right)\!\!\!\!\left(\displaystyle\sum_{i_1,\dots,i_H=1}^{\Lambda}\!\!\!\!\!r_{i_1,\dots,i_H}^2\prod_{k = 1}^{H}w_{i_k}^2\right)}}\\

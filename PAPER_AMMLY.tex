...
\begin{theorem}
Let $\mathcal{N}$, whose output is $Y_{\Lambda}$, be the neural network predicting $1$ with probability $p$ (without loss of generality assume $p \leq 0.5$) and $-1$ otherwise. Let $\mathcal{M}$ be its $(s,\epsilon)$-reduction image for some $s \leq \Lambda$ and $\epsilon \in [0,p)$. Then

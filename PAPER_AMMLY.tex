...
\aistatstitle{Connecting deep networks with the spin-glass model}
...
\begin{abstract}
We explore the approximate equivalence of the highly non-convex loss function of a simple model of the fully-connected feed-forward neural network with the Hamiltonian of the spherical spin glass model under a certain assumptions. Using the results from the random matrix theory, we demonstrate that for large-size near-symmetric networks the lowest critical values above the global minimum of the loss function are located in a well-defined band and furthermore they form a layered structure. We show that the expected number of local minima of the loss function in this band grows exponentially with the size of the network, and at the same time the probability of finding a saddle point in this band diminishes exponentially. We empirically demonstrate that both simulated annealing and SGD converges to the band of the lowest critical values, and furthermore all critical points found there correspond to the same learning quality measured by the test error.
\end{abstract}
...
\caption{Functions $\Theta_H(u)$ and $\Theta_{k,H}(u)$ for $k = \{0,1,\dots,5\}$. Parameter $H$ was set to $H = 3$. Black line: $u = -E_0(H)$, magenda line: $u = -E_{\infty}(H)$.}
\label{fig:Thetas}
\end{figure*}
...
\caption{Distribution of the mean number of critical points and local minima. Parameters $H$ and $\Lambda$ were set to $H = 3$ and $\Lambda = 1000$. Black line: $u = -E_0(H)$, magenda line: $u = -E_{\infty}(H)$, blue line: $u = 0$.}
...
\caption{Distribution of the mean number of local minima and low-index saddle points (oryginal and zoomed). Parameters $H$ and $\Lambda$ were set to $H = 3$ and $\Lambda = 1000$. Black line: $u = -E_0(H)$, magenda line: $u = -E_{\infty}(H)$.}

...
\paragraph{Input decomposability}
We assume each input $X_i$ is a normal random variable such that $X_i\sim\mathcal{N}(0,1)$ and it can be decomposed as follows: 
\[X_i = \frac{1}{\sqrt{v_i}}\sum_{j=1}^{v_i}Z_i^{(j)}
\]
for some positive $v_i$ and $Z$'s being independend identically distributed standard normal random variables. Furthermore we assume there exists more than one such decomposition. We will denote the set of all values of $v_i$, each taken from different decomposition, as a \textit{decomposition set of $X_i$}. 
...
\begin{eqnarray*}
\sum_{j=1}^{r_{i_1,\dots,i_H}^{'}}X_{i_1,\dots,i_H}^{(j)} &\!\!\!\!\!=& \!\!\!\!\!\sum_{j=1}^{r_{i_1,\dots,i_H}^{'}}\hat{r}_{i_1,\dots,i_H}\sum_{k=1}^{\frac{1}{\hat{r}_{i_1,\dots,i_H}^2}}Z_{i_1,\dots,i_H}^{(k,j)}\\
